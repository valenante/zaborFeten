...
const FacturasPage = () => {
  ...
  `);
    ...
    const verRespuestaAEAT = (xml) => {
        if (!xml) {
            setMensajeAlerta({
                tipo: "error",
                mensaje: "No hay respuesta de la AEAT disponible para esta factura.",
            });
            return;
        }
        ...
        // --- Formatear XML con sangría (mismo formato que verXML) ---
        const formatXML = (xmlString) => {
            try {
                const parser = new DOMParser();
                const xmlDoc = parser.parseFromString(xmlString, "application/xml");
